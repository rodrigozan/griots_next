...
const ListChapters = ({ id }) => {
  ...
  const fetchChapters = async () => {
    try {
      const response = await axiosInstance.get(`/api/books/${id}/chapters`);
  ...
  const handleDeleteSelected = async () => {
    try {
      for (const chapterId of selectedChapters) {
        await axiosInstance.post(`/api/books/${id}/chapters/delete/${chapterId}`, { bookID: id, chapterId: chapterId });
      }
  ...
  const handleDelete = async () => {
    ...
    axiosInstance.delete(`/api/books/${id}/chapters/${chapterId}`)
      .then(success => {
        console.log(success)
        setAlertMessage(SuccessDeletedMessage)
        setVariant('success')
